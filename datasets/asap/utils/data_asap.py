...
def get_dataset(name, sparse=True, cleaned=False, normalize=False):
    dataset = TUDataset('datasets/pyg/', name, use_node_attr=True, cleaned=cleaned)
    ...
    if dataset.data.x is not None and normalize:
    ...
    dataset.transform = T.Compose([dataset.transform, T.ToSparseTensor()])
